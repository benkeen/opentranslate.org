...
?>
  ...
  <h1><?=$LANG["label_make_translation"]?></h1>
  ...
  <div id="tab1_content" <?php if ($curr_tab != 1) echo "style=\"display: none;\""; ?>>
    ...
    <form action="<?=$_SERVER['PHP_SELF']?>" method="post" name="translation_form">
      ...
      <input type="hidden" name="origin_language_id" value="<?=$project['origin_language_id']?>" />
			...
			<?php
			// translations can only ever be updated if (a) they were originally provided by Google Translate or (b) it's the
			// original translator returning to translate the item
			if (empty($trans)) {
		  ...
		  else
			{
			  ...
			  echo "<input type=\"hidden\" name=\"translation_id\" value=\"{$trans["translation_id"]}\" />";
			}
			?>
      ...
      <tr>
        ...
        <td class="translation_row" style="padding-right:8px;">
          ...
          ?>
          <textarea style="width:100%;height:<?=$textarea_height?>" name="translation" id="translation"><?=@$trans["translation"]?></textarea>
        </td>
      </tr>
      </table>
